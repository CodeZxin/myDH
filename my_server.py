...
class MyServer:
            ...
            if len(self.__listCmd) > 0:
                ...
                if message:
                    ...
                    if username is None:
                        ...
                        tasks = [self.send_message_with_timeout(client, message, username, timeout=3) for client in
                                 wsclients]
    ...
    async def __handler(self, websocket, path):
        ...
        # 从客户端列表中移除已断开的连接
        await self.remove_client(websocket)
        print("websocket连接断开:{}".format(unique_id))
